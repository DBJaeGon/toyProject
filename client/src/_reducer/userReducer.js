...
export const fetchSignUp = createAsyncThunk(
  "user/signUpUser",
  async (signUpInfo, { rejectWithValue, getState, requestId }) => {
    try {
...
export const fetchSignIn = createAsyncThunk(
  "user/signInUser",
  async (signInInfo, { rejectWithValue, getState, requestId }) => {
    try {
...
// export const fetchGoogleOAuth = createAsyncThunk(
//   "/api/user/googleOAuth",
//   async (oauthInfo, { rejectWithValue, getState, requestId }) => {
//     try {
//       const { isLoading, signInState } = getState().user;
//       if (!isLoading || signInState.currentRequestId !== requestId) return;
//       const { data } = await axios.post("/api/users/socialSignIn", oauthInfo);
//       return data;
//     } catch (error) {
//       console.log(error);
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //
    // builder
    //   .addCase(fetchGoogleOAuth.pending, (state, action) => {
    //     if (!state.isLoading) {
    //       state.isLoading = true;
    //       state.signInState.currentRequestId = action.meta.requestId;
    //     }
